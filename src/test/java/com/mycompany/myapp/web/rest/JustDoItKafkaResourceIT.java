...
class JustDoItKafkaResourceIT {
    ...
    void setup() {
        ...
        JustDoItKafkaResource kafkaResource = new JustDoItKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/just-do-it-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/just-do-it-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
